...
Elm.Main.make = function (_elm) {
   ...
   var scene = $View.renderLayout(_L.fromArray([$View.menu
                                               ,$View.body]));
   var main = A3($Html.toElement,
   1024,
   768,
   scene);
...
Elm.View.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var renderLayout = function (children) {
      return A2($Html.div,
   ...
   var NoOp = {ctor: "NoOp"};
   var updatesChannel = $Signal.channel(NoOp);
   var menu = A2($Html.nav,
   ...
   _L.fromArray([$Html.text("Sample App")]))]))]))]));
