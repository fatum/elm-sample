...
Elm.Main.make = function (_elm) {
   ...
   var stringInput = function (string) {
      ...
      _L.fromArray([$Html$Attributes.placeholder("Text to reverse 2")
                   ,$Html$Attributes.value(string)
                   ,A3($Html$Events.on,
                   "input",
                   $Html$Events.targetValue,
                   function ($) {
                      return $Signal.send(updates)($Basics.identity($));
                   })
                   ,myStyle]),
      _L.fromArray([]));
   };
   ...
   var scene = function (string) {
      return A3($Html.toElement,
      400,
      200,
      stringReverser(string));
   };
   var main = $Signal.map(scene)($Signal.subscribe(updates));
